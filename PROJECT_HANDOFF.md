...
Run the packaged client's full sync screen once with the existing game directories and record its result; the read-only preflight predicts 18 map skips and 6 MOD skips, with zero installs. If the screen reports any planned replacement, inspect that item and preserve its local files before proceeding. A catalog item without a content fingerprint cannot prove that an edited same-version local folder matches the cloud package; automatic sync deliberately leaves that folder alone.
